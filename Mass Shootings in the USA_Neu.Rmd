...
gtrends$keyword <- ifelse(gtrends$time == "2004-03-12 2004-03-26", 
                             gsub("Fresno shooting", "Fresno shooting 1", gtrends$keyword, fixed = TRUE),
                             gtrends$keyword)

gtrends$keyword <- ifelse(gtrends$time == "2006-03-25 2006-04-08", 
                             gsub("Seattle shooting", "Seattle shooting 1", gtrends$keyword, fixed = TRUE),
                             gtrends$keyword)

gtrends$date <- gtrends[gtrends$date != "2009-01-08" || "2009-01-09",]
...
gtrends[i] <- subset(gtrends, keyword == "i")
gtrends[i] = 1:nrow(gtrends[i])
reg <- lm(hits ~ rownumber, data = gtrends[i], )
coef <- summary(reg)$coefficients[2]
gtrends[i]$coef <- coef
gtrends_Columbus <- rbind(gtrends_Columbus, gtrends[i])
}
