...
Tabelle mit für Missing, welche observations haben wir warum gedropped, wie viele observations verbleiben in unserem sample
...
for (i in listshooting){
    massshooting <- rbind(i)
 }

final <- dataframe()
for (i in gtrends$keyword) {
   gtrends_[i] <- subset(gtrends, keyword == "i")
   gtrends_[i] = 1:nrow8gtrends_[1]
   reg <- lm(hits ~ rownumber, data = gtrends-[1], )
   coef <- summary(reg)$coefficients[2]
   gtrends_[i]$coef <- coef
   final <- rbind(final, gtrends_[i])
}

massshooting <- rbind(listshooting[[1]]$interest_over_time, listshooting[[8]]$interest_over_time, listshooting[[15]]$interest_over_time, listshooting[[22]]$interest_over_time)
