...
In order to understand why people tend to concentrate on more salient events we have to look into some theories which explain this circumstance, but also the part of the media in it.
...
However, the fourth stage will diminish this. After the realization there will be a gradual decline of interest in this topic. Less news coverage and therefore less attention of people in general. 
...
In conclusion, even though the ultimate goal was not achieved, media attention and therefore public attention can has a great influence in solcing problems in their society. However, the event has to be large enough in order to start an issue-attention cycle. If not, the interest will be diminish even faster or will not be addressed at all. 
...
boxdead <- 
  ggplot(Final) +
  labs(title = "Boxplot of the Number of Dead Persons", x = "", y = "Number of Dead Persons") +
...
boxinjured <- 
  ggplot(Final) +
  labs(title = "Boxplot of the Number of Injured Persons", x = "", y = "Number of Injured Persons") +
...
#Versuch
#histdead <- ggplot(Final) +
#  geom_histogram(mapping = aes(x = Dead), binwidth = 1)

#histdead

# Bar plots of all categorical variables (Ethincity, Area, Gunlaws)

## Ethnicity
bareth <- ggplot(Final, aes(x = Ethnicity)) + # include percent
  labs(title = "Frequencies by Ethnicity", x = "Ethnicity", y = "Count") +
  geom_bar() +
  ...
  #geom_text(aes(label=paste0(round(Ethnicity*100,1),"%"),  y=pc))
  #geom_text(aes(label = ..prop..), stat= "count", vjust = -.5)
bareth

## Area
bararea <- ggplot(Final, aes(x = Area)) + # include percent
  labs(title = "Frequencies by Affluence of Community", x = "Affluence of Community", y = "Count") +
  geom_bar()  +
...
## Gunlaws
bargun <- ggplot(Final, aes(x = Gunlaws)) + # include percent
  labs(title = "Frequencies by Gun Law Strength", x = "Gun Law Strength", y = "Count") +
  geom_bar()  +
...
ggarrange(bareth, bararea, bargun + rremove("x.text"), 
          #labels = c("A", "B", "C"),
          ncol = 2, nrow = 2)
