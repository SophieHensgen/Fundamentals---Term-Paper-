...
```{r echo=FALSE}
ggplot(Final) +
  geom_point(aes(x = Dead , y = coef, color = Area)) +
  geom_smooth(aes(x = Dead , y = coef))

ggplot(Final) +
  geom_point(aes(x = Dead , y = coef, color = Gunlaws)) +
  geom_smooth(aes(x = Dead , y = coef))

ggplot(Final) +
  geom_point(aes(x = Dead , y = coef, color = Ethnicity)) +
  geom_smooth(aes(x = Dead , y = coef))


ggplot(Final) +
  geom_point(aes(x = Injured , y = coef, color = Area)) +
  geom_smooth(aes(x = Injured , y = coef))

ggplot(Final) +
  geom_point(aes(x = Injured , y = coef, color = Gunlaws)) +
  geom_smooth(aes(x = Injured , y = coef))

ggplot(Final) +
  geom_point(aes(x = Injured , y = coef, color = Ethnicity)) +
  geom_smooth(aes(x = Injured , y = coef))
```
...
```{r, echo=FALSE }
# Regression with Deads -- Interaction

#With interactive to TRUE we can observe single observations and get more information

#Gun Laws
regmixG <- lm(coef ~ Dead*Gunlaws, data = Final, )

ggPredict(regmixG,se=TRUE,interactive=FALSE, colorAsFactor = TRUE)

#Area
regmixA <- lm(coef ~ Dead*Area, data = Final, )

ggPredict(regmixA,se=TRUE,interactive=FALSE, colorAsFactor = TRUE)

#Ethnicity
regmixE <- lm(coef ~ Dead*Ethnicity, data = Final, )

ggPredict(regmixE,se=TRUE,interactive=FALSE, colorAsFactor = TRUE)


#Regression with Injured - Interaction

#Gun Laws
regmixGU <- lm(coef ~ Injured*Gunlaws, data = Final, )

ggPredict(regmixGU,se=TRUE,interactive=FALSE, colorAsFactor = TRUE)

#Area
regmixAR <- lm(coef ~ Injured*Area, data = Final, )

ggPredict(regmixAR,se=TRUE,interactive=FALSE, colorAsFactor = TRUE)

#Ethnicity
regmixET <- lm(coef ~ Injured*Ethnicity, data = Final, )

ggPredict(regmixET,se=TRUE,interactive=FALSE, colorAsFactor = TRUE)
...
# Möglich aber nicht nötig -- Interaktionen
stargazer::stargazer( regfull, regmixGU, regmixAR, regmixET, 
                     title   = "These are awesome results!", 
                     covariate.labels = c("Dead", "Injured", "Ethnicity","Area", "Gunlaws", "Injured x Gunlaws", "Injured x Area", "Injured x Ethnicity"), 
                     dep.var.labels   = "decrease of interest", 
                     keep.stat = c("adj.rsq", "n"),
                     flip = TRUE)  

stargazer::stargazer( regfull, regmixG, regmixA, regmixE, 
                     title   = "These are awesome results!", 
                     covariate.labels = c("Dead", "Injured", "Ethnicity","Area", "Gunlaws", "Dead x Gunlaws", "Dead x Area", "Dead x Ethnicity"), 
                     dep.var.labels   = "decrease of interest", 
                     keep.stat = c("adj.rsq", "n"),
                     flip = TRUE)  
```
...
ggPredict(regdead,se=TRUE,interactive=FALSE)
ggPredict(reginjured,se=TRUE,interactive=FALSE)

# Regression with Deads
