...
ggplot(example, aes(x = rownumber, y = hits, col = keyword))+
  ...
  labs(title = "Figure 1: Google Trend Example", subtitle = "Sandy Hook Elementary School Shooting & Clackamas Town Center shooting", x = "Days", y = "Search Hits", colour = "Keyword")+
...
On september 6 1949 one of the first publicly known mass shootings took place. One man strolled through his neigbourhood and killed 12 people, this shooting was later known as the walk of death[@Berger49]. Since then hundreds of shootings followed [@gva20] and even though it was not the first mass shooting, it was the first one that got broad media attention. But why do only some mass shootings get this kind of media attention and others are not even covered by one newspaper? In order to answer this question and therefore understand why people tend to concentrate on more salient events we have to inspect the course of how news are perceived and what they induce in oneself.

## Issue-Attention-Cycle

As we mentioned before, the media has a great impact on the information we consume. It dictates what will appear on our TV Screen and our newspaper. However, this is not a one way relation. The media also depends on what people want to see, which is something new, never heard before. People tend to get bored easily, even if a continuous problem occurs on their radar, after a while they strive for something new[Cite]. Therefore, news outlets have to be careful on what they are reporting in order to maintain their listeners. 

But why do people loose their interest in topics, even if they are still prominent? @Downs72 Issue-Attention-Cycle could answer this question. This cycle has five stages which I will explain by using the Parkland shooting as example. 
...
Not all mass shootings will live through the whole issue-attention cycle[Cite], moreover some mass shootings will loose the attention as fast as they gained it. Not because people realize how hard it would be to make a change in order to solve the problem, but because other events will occurre which are more interesting[Cite]. 
The moral panic theory describes this phenomenon, it has a similar structure if compared to the issue-attention cycle, however, it is much more ephemeral. Moral panics are short lived, intense media coverage will appear shortly after an event occurred (similar to the issue-attention cycle), but diminish almost as fast as they occurred[Cite]. Events have to be "triggering, focussing & defining"[Cite] in a certain way to gain the amount of media coverage, only then people will engage in it. However, this event will be replaced after only few days[Cite]. Most moral panics will peak in their first two days, after 5 to 6 days they're disappeared. Some moral panics can become routinized or institutionalized [cite!!], but most of them won't.


## Salience of events - newsworthiness

It seems to be that the most newsworthy events are events that are highly salient. But what do we perceive as highly salient? To begin iwth, it has to be something that does not happen often, a rare event[Cite]. If we remember back to the beginning of the paper, we mentioned, that 417 shootings occurred in 2019, only some were featured in the news[Cite]. That's because they are not newsworthy enough. The media would overflow the public with these shootings, sooner or later no one would read these articles anymore, as it would feel repetitive. So they feature only the exceptional events. 
Another important aspect is, that it has to be quite big and attention seeking[Cite]. An event that is maybe rare but small, will not get the same attention as a huge event would. For example, it is rather rare that a mass shooting is done by a women, which is a rare event in itself, however if she would only shot four people it would not get the same attention if any as if she shot more. 
And even if these aspects are fulfilled, it does not mean, that the event will be covered by the news as intensely as others because of the following reasons. First, if it is a problem by a minority it will be less salient and therefore less likely to be broadcasted. Second, if the problems which occurred give significant benefit to the majority or a powerful minority (e.g. pro gun allience) it will be less salient. And third, the event gave light to a problem, that has no (longer) intrinsically existing qualities[Cite]. 
...
```{r Micro-Macro-Model, echo=FALSE, fig.cap="Figure 2: Micro-Macro-Model", out.width = '90%'}
...
In the following our hypotheses will be analyzed with an OLS regression of the independent variables - the number of dead persons, the number of injured persons, the ethnicity of the shooter, the affluence of the community, and the strength of the states' gun laws -  on the decline in interest of mass shootings. According to hypothesis 1 and 2, we expect that the interest in the shooting declines slower, the more people died or got injured in the shooting. We also assume in line with hypotheses 3 that the interest in a mass shooting decreases slower if the shooter was black or had a migration background. Furthermore, we hypothesize (H4) that if the community the shooting happened in is more affluent, the decrease in interest is less steep. And finally we expect that according to hypothesis 5 that if the gun laws of the state in which a shooting took place are weak, the decline in interest is slower. 

The predicted scatterplots of the regressions show that the regression lines of the simple regression models between each of the independent variables and the dependent variable, the steepness of the decline in interest, seem to be in line with our hypotheses except for hypothesis 1. The regression line for hypothesis 1 shows that the interest decreases faster the more people die in a mass shooting. For all other hypotheses the regression line seem to support our hypothesis. --> Für andere hypothesen noch genauer beschreiben?
...
# Conclusion 
First we illustrated theoretical approaches which could explain how fast the interest in mass shootings declines. We highlighted the issue attention cicle, the moral panic theory, and the salience or newsworthyness of events. Based on these thories we argued that certain characteristics of mass shootings could influence how fast the interest declines. We derived five hypotheses which were analyzed by scraping data about mass shootings from Wikipedia and the relative search hits count for each shooting from Google Trends. Subsequently, we regressed this the steepness of the decline in interest on Google on the independent variables which were the number of dead persons, the number of injured persons, the ethnicity of the shooter, the affluence of the community, and the strength of the states' gun laws. The results show that hypotheses 1, 2 and 4 for have to be rejected. There does not seem to be a relationship between the number of dead persons, the number of injured persons and the affluence of the community and the steepness of the decline in public interest. In contrast hypotheses 3 and 5 cannot be rejected at the 10% level. If the perpetrator is non-white or has a migration background, the interest in the shooting declines slower than if the shooter is a white US American. This could be explained with the selection bias of the media when it comes to race and the subsequent higher public interest in the shooting. Furthermore, if the gun laws in the state a mass shooting happens in are weak the interest decreases slower than for states with strong gun laws. Potentially, this could be because the issue of gun control becomes more salient and the media covers the evnt more heavily. Thus, the public interest might also decrease slower. 

There are several important limitations to this study. The independent variables only explain a small amount of variation in the steepness of the decline in interest as the adjusted R^2 is very small. Moreover, the coefficients for the ethnicity of the shooter and the strength of the gun laws of the state in which a shooting happens in Model 6 are only significant on the 10% level. This poor model fit could be due to characteristics of mass shootings that are not accounted for in this analysis as well as well as certain data constraints which are illustrated in the following.
...
Furthermore, the Google Trends data constrained the operationalization of our dependent variable. Google Trends only reports the relative frequency of the search hits on Google in relation to the day in the time span that has the highest count of hits. Hence, we were only able to use the steepness of the decline in interest as the dependent variable. If we would have had the actual number of search hits per day we could have observed the actual interest in a mass shooting and not merely the steepness of the decline in interest. Future research about the public interest in mass shooting could try to operationalize the dependent variable in a more precise way and possibly include more covariates which could also influence the public interest in mass shootings. 
